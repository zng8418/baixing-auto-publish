...
SCENE_KEYWORDS = {
    ...
    "价格优势": ["128万", "1.07万/平", "宝安低价", "深圳低价盘", "个人出售"],
...
DESC_SECTIONS = {
    "opening": [
        "【{区域}·{小区名}·{户型}·{面积}平出售】",
        "{区域}{小区名} {户型} {面积}平 精选好房",
        "深圳{区域}{小区名} {户型}{面积}平方米 个人出售",
        "【{区域}核心地段】{小区名} {户型} {面积}平",
        "{区域}{街道} {小区名} {户型}大户型 {面积}平",
        "深圳{区域}{小区名}B栋 {户型} {面积}平诚意出售",
        "【个人出售 无中介费】{区域}{小区名} {户型} {面积}平",
    ],
    "basic_info": [
        "\n\n【基本信息】\n户型：{户型}\n面积：{面积}平方米\n楼层：电梯{楼层}楼\n朝向：{朝向}\n装修：{装修}（已通天然气管道）\n总价：{价格}万元（单价约{单价}万/平）\n产权：{产权}",
        "\n\n【房屋详情】\n{户型} | {面积}㎡ | 电梯{楼层}楼 | {朝向}\n毛坯交付，已铺设天然气管道\n总价{价格}万 | {产权}",
...
class SEOContentGenerator:
    ...
    def generate_title(self, day, seed=None):
        ...
        fill = {
            ...
            '卖点': random.choice(['个人出售', '带电梯', '南北通透', '天然气入户']),
        }
    ...
    def generate_description(self, day, seed=None):
        ...
        # 联系方式已移除 — 仅用表单 contact 字段（白名单）
        # 描述中不放任何联系方式，避免触发百姓网反垃圾
    ...
    def seo_score(self, title, description):
        ...
        # 7. 联系方式: 不再在描述中放联系方式（百姓网反垃圾检测）
        # contact 字段通过表单白名单提交，描述中不放
        score += 5
        details.append("✅ 联系方式仅通过表单字段提交（安全）")
